...
public class Excelread {
	...
	public List<String> dataread(String testcasename) throws IOException
	{
		...
		FileInputStream fis = new FileInputStream("File");   // Pass the excel path in the argument
		XSSFWorkbook workbook = new XSSFWorkbook(fis); // Get access to the excel workbook
			...
			if(workbook.getSheetName(sheetcount).equalsIgnoreCase("sheet_name"))
			{
				...
				//int rowcount= worksheet.getLastRowNum();
				 ...
				 while(cellvalue.hasNext())
				 {
					 ...
					 if(value.getStringCellValue().equalsIgnoreCase("TestCase"))  // Passing the required column name from excel
						 column=temp;
				 }
				 temp++;
